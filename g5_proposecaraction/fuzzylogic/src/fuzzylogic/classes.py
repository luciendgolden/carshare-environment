...
### START OF CODE THAT ISN'T IN THE ORIGINAL LIBRARY
    def weights(self, args: "dict[Domain, float]"):
        """Calculate the weights for the target (output) domain
        """
        assert len(args) == max(
        ...
        for K, v in self.conditions.items():
            ...
            if x > 0:
                weights.append((v, x))
        return weights
### END OF CODE THAT ISN'T IN THE ORIGINAL LIBRARY
